          ...
          <div className="flex items-center min-w-[90px] sm:min-w-[120px] justify-end">
            <div className="flex items-center bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-full border border-purple-100 dark:border-purple-900/50 shadow-lg w-full h-[34px] sm:h-[38px]">
              <div className="flex items-center justify-between w-full px-3 h-full">
                ...
                <div className="h-4 w-px bg-purple-200 dark:bg-purple-800/50" />
                <a href="https://cineby.app" target="_blank" rel="noopener noreferrer" className="flex items-center hover:opacity-80 transition-all duration-200 h-full">
...
const Player = () => {
  ...
  const playerUrl = () => {
    ...
    const colorParam = `?color=${selectedColor.replace("#", "")}`;
    const featuresParams = `&nextEpisode=true&episodeSelector=true&autoplayNextEpisode=true`;
...
// New component to wrap Routes for AnimatePresence
const RootRouterContent = () => {
  const location = useLocation(); // Now useLocation is correctly inside the <HashRouter> context
  ...
  return (
    <AnimatePresence mode="wait" initial={false} onExitComplete={() => window.scrollTo(0, 0)}>
      <Routes location={location} key={location.pathname}>
        ...
        <Route path="/FreeTV" element={<FreeTV />} />
        ...
        <div className="min-h-screen w-full bg-gradient-to-b from-gray-950 via-purple-950 to-gray-950">
          ...
          <div className="relative">
            ...
            <main className="relative min-h-[calc(100vh-4rem)]">
              <RootRouterContent /> {/* Render the new component here */}
            </main>
          </div>
        </div>
      </div>
